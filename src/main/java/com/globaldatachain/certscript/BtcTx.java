...
public class BtcTx implements Instruction {
    private final BitcoinTransactionRetriever bitcoinTransactionRetriever;

    public BtcTx(BitcoinTransactionRetriever bitcoinTransactionRetriever) {
    ...
    @Override public void apply(Deque<byte[]> stack) throws ScriptFailedException, EmptyStackException {
        try {
            byte[] txid = stack.pop();
            stack.push(bitcoinTransactionRetriever.getTransaction(txid));
